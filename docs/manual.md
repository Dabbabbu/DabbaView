...
버전 2.18.0 · macOS / Windows · 변경 이력은 [CHANGELOG](../CHANGELOG.md)
...
4. 클라우드 폴더는 **Open from Google Drive… / Open from OneDrive…** 로 엽니다. 각자의 OAuth 키를 Settings → Cloud에 먼저 넣어야 합니다.
   ...
   - **⚡ 빠른 열기**(권장 · 기본 켜짐): 파일 앞부분 64 KB(메타데이터)만 먼저 받아 시리즈 목록을 바로 띄우고, 영상을 여는 순간 그 파일만 통째로 받습니다. 안 보는 시리즈는 아예 받지 않습니다. 동기화 폴더로 열 때는 운영체제가 한 바이트만 읽어도 파일 전체를 받아버려 불가능하고, **API로 연결했을 때만** 됩니다.
