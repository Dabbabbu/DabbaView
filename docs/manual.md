...
버전 2.19.0 · macOS / Windows · 변경 이력은 [CHANGELOG](../CHANGELOG.md)
...
4. 클라우드 폴더는 **Open from Google Drive… / Open from OneDrive…** 로 엽니다. 각자의 OAuth 키를 Settings → Cloud에 먼저 넣어야 합니다.
   - 영상과 같은 폴더에 있는 **판독문 `.txt`**는 함께 받아 그 검사(Study)의 판독문으로 연결됩니다 (📝 Reading에서 확인). 이미 작성한 판독문이 있으면 덮어쓰지 않습니다.
   - 폴더를 고르면 **폴더 수 · 파일 수 · 용량**이 요약으로 나오고, **▸ 자세히**로 확장자별 개수·용량을 펼쳐 볼 수 있습니다.
   - **열기 방식**을 고릅니다: **⚡ 빠른 열기(권장)** 또는 **⬇ 전체 다운로드(오프라인 대비)**. 고를 때마다 예상 용량이 표시됩니다.
     - **⚡ 빠른 열기**: 파일 앞부분 64 KB(메타데이터)만 먼저 받아 시리즈 목록을 바로 띄우고, 영상을 여는 순간 그 파일만 통째로 받습니다. 안 보는 시리즈는 아예 받지 않습니다. 동기화 폴더로 열 때는 운영체제가 한 바이트만 읽어도 파일 전체를 받아버려 불가능하고, **API로 연결했을 때만** 됩니다.
   - 시리즈를 열면 나머지 영상을 **백그라운드로 미리 받아** 스크롤이 끊기지 않습니다. 인터넷이 없는 곳에서 보려면 **File ▸ ☁ 클라우드 영상 전체 받기…** 로 미리 받아 두세요 (상태바의 **☁ N장 미수신**을 눌러도 됩니다). MPR·3D·일괄 동영상처럼 전체가 필요한 기능은 시작할 때 알려 줍니다.
   - 받은 파일은 **다운로드 폴더 아래 날짜 폴더**에 저장됩니다(기본 `~/Downloads/DabbaView`, Windows는 `다운로드\\DabbaView`). 탐색 창 아래 **저장 위치** 줄에서 바로 바꾸거나 Settings ▸ Cloud에서 지정할 수 있습니다.
...
**File ▸ Batch Export Videos…** 로 여러 시리즈를 한 번에 동영상으로 만듭니다.

1. 목록에서 시리즈를 체크합니다 (2장 이상인 시리즈만 나오고, 기본은 전체 선택). **전체 선택 / 전체 해제** 버튼이 있고, 아래에 `선택 3 / 10 시리즈 · 총 69장`처럼 표시됩니다.
2. 형식(**MP4 · AVI · GIF**), 재생 시간(초, 시리즈마다 같은 길이로 맞춤), 해상도(**원본 / 512×512 / 1024×1024**)를 고릅니다.
3. **현재 화면의 W/L 적용**을 끄면 시리즈마다 밝기를 자동으로 잡습니다.
4. 저장 폴더를 고르고 **내보내기**를 누르면 `3/10 시리즈 변환 중…`처럼 진행 상황이 나옵니다.
5. 파일 이름은 `{시리즈 설명}_{순번}.mp4`입니다. 순번은 이번 내보내기의 순서(001, 002, 003 …)이고, 같은 이름이 있으면 뒤에 `_2`가 붙습니다.
   - 예: `Ax T2_001.mp4`, `SA CINE_002.mp4`, `2 CH CINE_003.mp4`, `SAx T2Map BH_004.gif`
   - 파일 이름에 쓸 수 없는 문자(`/ \ : * ? " < > |`)는 `_`로 바뀝니다.
   - 한 시리즈만 내보내는 **Export as Video…** 는 번호 없이 설명만 씁니다 (`Ax T2.mp4`).
6. 한 시리즈가 실패해도 나머지는 계속 만들고, 끝나면 성공·실패 개수를 알려 줍니다. 닫기를 누르면 진행 중인 작업을 멈춥니다.
...
2. Report 탭에 소견과 결론(Conclusion)을 씁니다. 작성자·승인자·검사일이 함께 저장됩니다. (화면은 CMR 판독 틀 예시)
...
**Help** 메뉴에서 고르면 브라우저에 GitHub Issue 작성 창이 양식과 함께 열립니다 (GitHub 로그인 필요).

| 메뉴 | 쓸 때 | 양식 |
|---|---|---|
| 🐛 버그 리포트 | 동작이 이상하거나 오류 창이 뜰 때 | 증상, 재현 방법, 기대 동작, 스크린샷, 환경(버전 · OS · 영상 종류) |
| 💡 기능 요청 | 있었으면 하는 기능 | 기능 설명, 이유 · 용도, 대안, 참고 자료 |
| 📋 질문하기 | 사용법이 궁금할 때 | 질문 내용, 관련 기능, 이미 확인한 문서, 환경 |

- 버전은 **Help ▸ About DabbaView**에서 확인해 적어주세요.
- 스크린샷을 올릴 때는 **환자 정보가 보이지 않도록** 가린 뒤 올려주세요. 이슈는 누구나 볼 수 있습니다.
...
1. 탭 이름이나 패널 제목에 **마우스를 올리면 ✕ 버튼**이 나옵니다 (평소에는 숨김). 누르면 잠깐 흐려지며 닫힙니다.
   - 대상: Series · ★ Library 탭, Image · AI · Analysis · ROI Manager · Histogram/Profile · Python Console 패널, 패널을 겹쳐 놓았을 때 생기는 아래쪽 탭, Reading 창의 붙인 파일 탭
...
| **원본에 덮어쓰기** | ⚠️ 원본 DICOM 파일에 주석을 넣습니다 (개인 태그 `(0071,xx01)`, 작성자 `DabbaView Annotations`). 파일마다 `.bak` 백업을 만듭니다 (Settings ▸ Reading의 백업 설정) |
