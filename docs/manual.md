...
버전 3.6.0 · macOS / Windows · 변경 이력은 [CHANGELOG](../CHANGELOG.md)
...
3. **가운데 버튼 드래그**로 이동하고, **Ctrl(⌘)+휠**로 확대합니다.
   - **영상 오른쪽 조절 막대**: 영상을 가리지 않는 세로 막대에 **🔍 Zoom**과 **◐ W/L** 칸이 있습니다. 누른 채 끌면 Zoom은 위로 확대 · 아래로 축소, W/L은 좌우 = Width · 위아래 = Level입니다. 끄는 동안 칸에 배율과 W/L 값이 나오고, 두 번 클릭하면 화면 맞춤 / DICOM 기본 W/L로 돌아갑니다. Multi View에서는 선택한 칸에 적용되고, MPR · 3D에서는 숨겨집니다. **View ▸ 🔍◐ 영상 옆 Zoom · W/L 조절 막대**로 끄고 켭니다.
