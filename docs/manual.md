...
버전 3.4.1 · macOS / Windows · 변경 이력은 [CHANGELOG](../CHANGELOG.md)
...
4. 클라우드 폴더는 **Open from Google Drive… / Open from OneDrive…** 로 엽니다. 각자의 OAuth 키를 Settings → Cloud에 먼저 넣어야 합니다.
   ...
   - 클라우드 창·불러오기 창은 **독립 창**이라 다른 프로그램 뒤로 가도 3D Volume 탭 옆의 **☁ 탭**을 누르면 다시 앞으로 나옵니다.
   - 불러오기 창과 클라우드 창 아래의 **끝나면 자동으로 닫기**를 끄면, 다 끝난 뒤에도 창이 남아 결과(파일 수 · 시리즈 수 · 걸린 시간)를 보여 주고 **닫기** 버튼으로 직접 닫습니다. 설정은 기억되어 두 창에 똑같이 적용됩니다.
