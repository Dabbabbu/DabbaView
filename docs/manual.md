...
버전 3.14.0 · macOS / Windows · 변경 이력은 [CHANGELOG](../CHANGELOG.md)
...
6. **다른 폴더의 시리즈를 더해서 나란히 보기**: **File ▸ 📂➕ Add Folder…** (Ctrl(⌘)+Alt+O)는 지금 연 영상은 그대로 두고 다른 폴더를 더해, Multi View의 빈 칸에 나란히 엽니다 (칸이 다 차면 격자가 한 단계 커짐). 같은 검사(같은 좌표계)면 **⌖ Crosslink**(스캔 범위) · **Ref Lines**(지금 슬라이스 위치)로 서로의 위치가 보입니다. 좌표계 표시가 없거나 다른 시리즈도 **View ▸ ⌖ Crosslink · Ref Lines 연동 기준**에서 `같은 검사면` 또는 `환자 좌표만 보고 항상`으로 바꾸면 연동됩니다 (다른 날 검사는 위치가 어긋날 수 있음). 위치 정보가 아예 없는 영상(판독 화면 · 캡처 등 Secondary Capture)은 연동할 수 없습니다.
