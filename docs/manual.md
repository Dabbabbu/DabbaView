...
버전 3.1.0 · macOS / Windows · 변경 이력은 [CHANGELOG](../CHANGELOG.md)
...
4. 클라우드 폴더는 **Open from Google Drive… / Open from OneDrive…** 로 엽니다. 각자의 OAuth 키를 Settings → Cloud에 먼저 넣어야 합니다.
   - **받을 파일 유형 고르기**: ▸ 자세히의 표에서 확장자마다 체크할 수 있습니다. 같은 검사가 DICOM과 JPG로 중복돼 있으면 **DICOM만** 버튼으로 한 번에 고르고, 고른 유형의 **개수 · 용량 · 예상 시간**이 바로 표시됩니다 (기본값: DICOM만).
   ...
   - 클라우드 창·불러오기 창은 **독립 창**이라 다른 프로그램 뒤로 가도 메인 창 오른쪽의 **☁ 탭**을 누르면 다시 앞으로 나옵니다. 창 제목에도 진행률이 표시됩니다.
