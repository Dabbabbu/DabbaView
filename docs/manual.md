...
5. 클라우드 폴더는 **Open from Google Drive… / Open from OneDrive…** 로 엽니다. 각자의 OAuth 키를 Settings → Cloud에 먼저 넣어야 합니다.
   ...
   - **☑ 여러 개 선택**: 목록 위 버튼을 켜면 폴더 · 파일 앞에 체크 상자가 생기고, 누를 때마다 체크/해제됩니다 (⌘ · Shift 키 없이 여러 개 고르기). **전체**로 모두 체크/해제, 고른 것은 **선택 항목 열기 (N개)**로 엽니다.
   - 클라우드 창은 실시간으로 바뀌지 않습니다. 그사이 드라이브에 파일을 넣었으면 경로 왼쪽 **🔄 새로 고침**(F5 · ⌘R)을 누르세요. 다른 창에 갔다가 20초 넘게 지나 돌아오면 자동으로 다시 확인하고, 새로 생긴 항목은 **🆕 초록색**, 상태 줄에 `새 항목 N개 · 없어진 항목 N개`로 알려 줍니다. 이미 받은 파일은 캐시에 있어 다시 열어도 새 파일만 받습니다.
   - 목록에 안 나오는 폴더(다른 컴퓨터 백업 등)는 위쪽 **🔍 폴더 이름으로 전체 검색** 칸에 이름을 치고 Enter로 찾습니다. 검색 결과에는 오른쪽 **위치** 칸에 그 폴더가 들어 있는 경로(예: `내 드라이브/때려넣기/…/heart`)가 나오고, 열면 위쪽 경로도 전체로 보입니다. 검색칸을 누르면 **최근 검색어**(최대 10개)가 뜨고, 누르면 다시 검색, 옆의 **✕**로 지우기, **모두 지우기**로 한꺼번에 지웁니다.
   ...
   - 받은 파일은 **다운로드 폴더 아래 날짜 폴더**에 저장됩니다(기본 `~/Downloads/DabbaView_downloads`, Windows는 `다운로드\\DabbaView_downloads`). 탐색 창 아래 **저장 위치** 줄에서 바로 바꾸거나 Settings ▸ Cloud에서 지정할 수 있습니다.
   - 목록에 없는 폴더는 탐색 창 위쪽 **🔍 칸에 이름을 넣고 Enter**로 찾습니다. 구글 드라이브 **'다른 컴퓨터'(백업된 PC)** 안의 폴더는 내 드라이브 목록에 나오지 않으므로 이 방법으로 엽니다.
6. **다른 폴더 · 파일을 더해서 나란히 보기**: **File ▸ ➕ 파일 추가…** (Ctrl(⌘)+Alt+Shift+O) · **➕ 폴더 추가…** (Ctrl(⌘)+Alt+O)는 Open File · Open DICOM Folder 바로 아래에 있고, 지금 연 영상은 그대로 두고 고른 파일(여러 개 · 압축파일 가능)이나 폴더를 더해, Multi View의 빈 칸에 나란히 엽니다 (칸이 다 차면 격자가 한 단계 커짐). 같은 검사(같은 좌표계)면 **⌖ Crosslink**(스캔 범위) · **Ref Lines**(지금 슬라이스 위치)로 서로의 위치가 보입니다. 좌표계 표시가 없거나 다른 시리즈도 **View ▸ ⌖ Crosslink · Ref Lines 연동 기준**에서 `같은 검사면` 또는 `환자 좌표만 보고 항상`으로 바꾸면 연동됩니다 (다른 날 검사는 위치가 어긋날 수 있음). 위치 정보가 아예 없는 영상(판독 화면 · 캡처 등 Secondary Capture)은 연동할 수 없습니다.
7. 최근에 연 파일은 **Recent Files**에 10개까지 남습니다.

> 손상되었거나 지원하지 않는 파일은 건너뛰고 오른쪽 아래 **⚠ 로딩 실패** 버튼에 목록으로 남깁니다.

**읽지 못한 파일 다시 읽기 · 일시정지** (네트워크 드라이브 · Google Drive · OneDrive처럼 느린 폴더)
- 파일 하나가 한도(내 디스크 10초, **네트워크 · 클라우드 폴더 30초**)를 넘기면 건너뛰고 나머지를 계속 읽습니다. 네트워크 · 클라우드 폴더는 동시에 읽는 파일 수를 줄여 한꺼번에 시간 초과가 나지 않게 합니다.
- 다 읽은 뒤 건너뛴 파일이 있으면 불러오기 창에 **⟳ 실패 N개 재시도** 버튼이 나옵니다. 누르면 한도를 **두 배**로 늘리고 천천히 다시 읽어, 읽힌 영상은 **원래 시리즈에 합쳐** 보여 줍니다(지금 보던 화면은 그대로).
- 읽은 시리즈는 목록에 그대로 나오고, **읽지 못한 파일은 시리즈 목록 아래 '⚠ 불러오지 못한 파일'** 상자에 폴더별로 따로 나옵니다. 폴더를 **더블클릭**하면 그 폴더만 다시 읽고, **⟳ 재시도**는 전부 다시 읽습니다. 같은 폴더에서 일부가 빠진 시리즈 카드에는 **⚠ N 누락** 표시가 붙습니다.
- ⚠ 로딩 실패 목록 창에서도 **⟳ 재시도**(여러 행을 고르면 그것만)를 할 수 있습니다. DICOM이 아니거나 손상된 파일은 다시 해도 같으므로 제외됩니다.
- 불러오는 동안 **⏸ 일시정지 / ▶ 계속**. 시스템 메모리가 한도(기본 90%)를 넘으면 **저절로 멈추고**, 메모리가 내려가면 다시 시작합니다. 바로 이어 가려면 **▶ 그래도 계속**.
- 한도는 **Settings ▸ 불러오기**에서 바꿉니다: 파일 하나 대기 시간(내 디스크 · 네트워크 · 클라우드 받기), 클라우드 연속 실패 시 멈출 개수(0 = 끝까지 시도), 메모리 일시정지 켜기 · 기준 %.
...
3. **가운데 버튼 드래그**로 이동하고, **Ctrl(⌘)+휠**로 확대합니다.
   - **맨 아래 Zoom · W/L 조절 칸**: 창 맨 아래 상태바, **뷰어 창 가로 가운데**에 **🔍 Zoom**과 **◐ W/L** 칸이 있어 (옆 패널을 열고 닫아도 가운데 유지) 영상 크기에 영향을 주지 않습니다. 누른 채 끌면 Zoom은 위로 확대 · 아래로 축소, W/L은 좌우 = Width · 위아래 = Level입니다. 끄는 동안 칸에 배율과 W/L 값이 나오고, 두 번 클릭하면 화면 맞춤 / DICOM 기본 W/L로 돌아갑니다. Multi View에서는 선택한 칸에 적용되고, MPR · 3D에서는 숨겨집니다. **View ▸ 🔍◐ 맨 아래 Zoom · W/L 조절 칸**으로 끄고 켭니다.
   - **좌클릭 + 우클릭을 함께 누른 채 위아래로 끌면** 슬라이스가 넘어갑니다. 천천히 끌면 한 장씩, **빠르게 확 끌면 한꺼번에 많이** 넘어갑니다 (최대 12배). 위상 영상(heart cine 등)에서는 **좌우로 끌면 위상**이 넘어갑니다. 한쪽 버튼을 먼저 떼도 남은 버튼으로 계속 넘길 수 있습니다.
   - **↑ ↓ 방향키**도 한 장씩 넘깁니다. **← →** 는 위상 영상에서만 위상을 넘깁니다.
4. **어디서 불러왔는지**: 시리즈 카드 맨 아래 `📁 폴더 이름`(압축파일이면 `묶음.zip ▸ 안쪽 폴더`), 창 제목 줄에 지금 시리즈의 폴더 이름, 맨 아래 상태바 왼쪽에 지금 슬라이스의 `📄 파일 이름`이 나옵니다. 카드나 파일 이름에 마우스를 올리면 전체 경로가 보입니다.
5. 네 모서리 오버레이에 환자·검사·시리즈·획득 정보가 나옵니다. **T** 키로 켜고 끕니다.
6. 영상 가장자리 가운데에 **방향 문자**(L/R · A/P · S/I)가, 위쪽 가운데에 **위상 인코딩 방향**이 나옵니다.
...
7. **Phase 버튼 띠** — cine · perfusion처럼 한 위치에 여러 장(시간 위상)이 있는 시리즈에서는 영상 위에 위상 번호 버튼이 나옵니다.
   ...
   - **방향키**: **↑ ↓** 슬라이스 위치 (위상 고정), **← →** 위상. 좌+우 버튼을 함께 누르고 끌 때도 같습니다 (위아래 = 슬라이스, 좌우 = 위상).
   - **View ▸ ↕ 방향키 ↑↓ 동작** 메뉴에서 ↑↓를 잠시 **전체 순서**(슬라이스 1의 위상 1 → … → 마지막 위상 → 슬라이스 2의 위상 1) 또는 **위상**으로 바꿀 수 있습니다. **Ctrl(⌘)+Shift+↑** 로 차례로 바꿀 수도 있고, 지금 동작은 띠 오른쪽에 `↑↓ 슬라이스 위치, ←→ 위상`처럼 나옵니다. 앱을 다시 켜면 '슬라이스 위치'로 돌아갑니다.
...
8. **▦ Tile** (Shift+T)을 누르면 여러 영상을 격자로 봅니다. 아래 예는 SAX cine의 연속 위상입니다. 격자 크기는 옆의 `4x` 목록에서 고릅니다.
...
| 기능 | 하는 일 |
|---|---|
| **Crosslink (C)** | **선택한 칸** 시리즈의 **전체 스캔 범위**를 다른 칸들 위에 파란 점선으로, 지금 슬라이스는 **노란 실선**으로 (선택한 칸에는 선 없음) |
...
2. PACS 스카우트처럼 **선택한 칸(노란 테두리)의 시리즈**가 기준입니다. 선택한 칸 자신에는 아무 선도 없고, 위치 정보가 연동되는 **다른 칸들 위에** 그 시리즈의 **전체 슬라이스(스캔 플래닝)가 파란 점선**으로, **지금 보고 있는 슬라이스가 노란 실선**(2 px)으로 그려집니다.
   - 예: SAX 칸을 선택하면 4CH · 2CH 칸에 SAX 슬라이스 10개가 파란 점선으로 나와, 단축 스택이 심장을 어디부터 어디까지 덮는지 한눈에 보입니다.
   - SAX 칸을 휠로 넘기면 노란 실선이 따라 움직입니다. 다른 칸을 누르면 그 칸이 기준이 됩니다.
   - 평행한 칸(axial ↔ axial)에는 선이 생기지 않습니다.
...
| Ctrl(⌘)+Shift+↑ | ↑↓ 동작 바꾸기 (슬라이스 위치 → 전체 순서 → 위상) | | |
| **Ctrl(⌘)+F** | **🔍 기능 찾기** | | |
