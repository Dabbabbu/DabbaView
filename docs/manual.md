...
버전 3.7.0 · macOS / Windows · 변경 이력은 [CHANGELOG](../CHANGELOG.md)
...
4. 클라우드 폴더는 **Open from Google Drive… / Open from OneDrive…** 로 엽니다. 각자의 OAuth 키를 Settings → Cloud에 먼저 넣어야 합니다.
   ...
   - 목록에 안 나오는 폴더(다른 컴퓨터 백업 등)는 위쪽 **🔍 폴더 이름으로 전체 검색** 칸에 이름을 치고 Enter로 찾습니다. 검색칸을 누르면 **최근 검색어**(최대 10개)가 뜨고, 누르면 다시 검색, 옆의 **✕**로 지우기, **모두 지우기**로 한꺼번에 지웁니다.
