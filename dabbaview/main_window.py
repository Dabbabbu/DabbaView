...
class MainWindow(QMainWindow):
    ...
    def _attach_text_reports(self, loader):
        """영상과 같은 폴더에 있던 .txt 기록을 해당 검사(Study)에 붙인다
        ...
        if attached:
            self._statusbar.showMessage(
                f"기록 파일 {len(attached)}개를 검사에 연결했습니다 "
                f"({', '.join(attached[:2])}{'…' if len(attached) > 2 else ''}) — "
                "📝 Reading에서 확인하세요.", 12000)
        elif skipped:
            self._statusbar.showMessage(
                f"기록 파일 {skipped}개를 찾았지만 이미 작성된 기록이 있어 두었습니다.", 8000)
    ...
    def _on_load_finished(self, loader, loaded):
        ...
        self._attach_text_reports(loader)     # 같은 폴더의 기록 .txt → 해당 검사에 연결
        notes = self._apply_loaded_extras(loader)
