...
class SettingsDialog(QDialog):
    ...
    def _work_tab(self):
        ...
        self._update_check = QCheckBox("시작할 때 새 버전 확인 (GitHub Releases)")
        self._update_check.setChecked(self._settings.update_check())
        self._update_check.setToolTip("앱을 열 때 최신 Release 태그만 확인합니다 (개인 정보는 보내지 않음).")
        layout.addWidget(self._update_check)
        skipped = self._settings.update_skip_version()
        if skipped:
            row4 = QHBoxLayout()
            row4.addWidget(QLabel(f"다시 알리지 않기로 한 버전: {skipped}"))
            clear = QPushButton("해제")
            clear.clicked.connect(lambda: (self._settings.set_update_skip_version(""), clear.setEnabled(False)))
            row4.addWidget(clear)
            row4.addStretch(1)
    ...
    def _reading_tab(self):
        ...
        self._report_folder.setPlaceholderText("기록 파일이 저장되는 폴더 (선택)")
    ...
    def _browse_report_folder(self):
        from PyQt5.QtWidgets import QFileDialog
        folder = QFileDialog.getExistingDirectory(self, "기록 폴더", self._report_folder.text())
        if folder:
