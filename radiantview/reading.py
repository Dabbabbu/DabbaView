"""
Reading(판독) 팝업 - INFINITT PACS 판독 창 형태
...
def default_reports_dir():
    ...
    return os.path.join(base or os.path.expanduser("~/.radiantview"), "reports")
...
class ReportStore:
    """StudyInstanceUID별 판독문 JSON 저장소"""
...
class ReportLibrary(QObject):
    """판독문 폴더를 감시해서 불러온 검사와 파일을 자동 매칭"""
...
class ImageViewer(QScrollArea):
    """판독문 이미지/PDF 뷰어 (Ctrl+휠·버튼으로 확대/축소)"""
...
class ReadingDialog(QDialog):
    ...
    def approve(self):
        if not self.report_text().strip() or self.report_text().strip() == REPORT_TEMPLATE.strip():
            QMessageBox.information(self, "Approve", "판독문 내용이 비어 있습니다.")
            return
