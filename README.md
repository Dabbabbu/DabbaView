...
<!-- version -->**Version** 2.12.0 — 버전은 `dabbaview/__init__.py`의 `__version__` 하나로 관리합니다 (앱 번들·타이틀 바·About·시작 화면이 이 값을 사용하고, 빌드할 때 이 줄도 자동으로 맞춰집니다).
...
| 문서 | Markdown | PDF |
|---|---|---|
| 사용자 매뉴얼 (설치 · 보기 · 측정 · ROI · Reading · Library · Settings · 단축키) | [docs/manual.md](docs/manual.md) | [manual.pdf](docs/manual.pdf) |
...
- **DICOM 파일 원본도 수정**(기본): pydicom으로 태그만 바꾸고 픽셀 데이터는 그대로, 진행률·취소, 수정 전 `<파일>.bak` 백업(Settings → Reading에서 끄기). 끄면 원본은 그대로 두고 Library에서만 표시 이름 변경
...
- **Reading**: 판독문 폴더(감시), 기본 Creator, DICOM 원본 수정 전 .bak 백업
